...
def add_thread_to_sheet_with_user_agent(thread_id, platform, user_agent_string, sheet, location_info, ip):
    try:
        ...
        row = [
            ...
            location_info.get("country", ""), location_info.get("region", ""),
            location_info.get("city", ""), os, device, ip
        ]
        sheet.append_row(row)
        logging.info("Thread added to sheet successfully with user agent data and location info.")
    except Exception as e:
        logging.error(f"An error occurred while adding the thread to the sheet: {e}")

def add_thread_to_airtable(thread_id, platform, user_agent_string, location_info, ip):
    ...
    data = {
        "records": [{
            "fields": {
                ...
                "Platform": platform,
                "Status": "Arrived",
                "País": location_info.get("country", ""),
                "Estado": location_info.get("region", ""),
                "Ciudad": location_info.get("city", ""),
                "OS": os,
                "Device": device,
                "IP": ip
            }
